...
function detectIntent(input) {
    ...
    const isContinue = /wait|actually|what if|i just noticed|but then|what happens next|keep going|and then|continue|more|make it|make the|scarier|funnier|interesting|sad(der)?|longer|shorter|rewrite|change it|edit|different ending/i.test(text);
    if (isContinue)
        return "continue";
    const isSuggest = /suggest|topic|idea|what (should|can) i (write|narrate|tell)|give me (a topic|ideas|some topics)|what (kind of story|stories)|which story|help me (choose|pick|decide)/i.test(text);
    if (isSuggest)
        return "suggest";
    const isStory = /write|tell me|generate|create|give me|make|story about|a story|horror story|love story|funny story|sad story|thriller|write a|narrate|poem about|flash fiction|diary entry|voice note|linkedin post|twitter thread|reddit post/i.test(text);
    if (isStory)
        return "story";
    const isOffTopic = /^(hi+|hey+|hello+|what'?s up|sup|yo|hola|namaste|good (morning|evening|night|afternoon)|how are you|what are you|who are you|what can you do|what to eat|food|weather|news|joke|meme|coding|math|help me with|explain|what is|how to|can you|do you)/i.test(text);
    if (isOffTopic)
        return "off_topic";
...
function buildConversationPrompt(input, intent) {
    if (intent === "suggest") {
        return `
...
`;
function getGenreInstruction(input) {
    if (/\blove story\b|falling in love|first love|unrequited|secret love/i.test(input))
        return `
GENRE: Love story.
...
Tone: warm, slightly nervous, achingly hopeful.`;
    if (/\bromance\b|romantic|slow burn|chemistry/i.test(input))
        return `
GENRE: Romance.
...
Tone: warm, fluttery, a little reckless.`;
    if (/breakup|break up|broke up|heartbreak|she left|he left|moving on|\bex\b|after us/i.test(input))
        return `
...
Tone: quiet, heavy, honest. End unresolved.`;
    if (/horror|scary|haunted|ghost|demon|cursed|paranormal|something wrong|mirror/i.test(input))
        return `
...
IMPORTANT: Horror needs at least 3 full paragraphs to build proper dread.`;
    if (/thriller|suspense|mystery|murder|missing|conspiracy|someone following/i.test(input))
        return `
...
Tone: tense, urgent, a little paranoid.`;
    if (/\bfunny\b|comedy|humor|hilarious|awkward|embarrassing moment/i.test(input))
        return `
GENRE: Comedy / humor.
...
Tone: warm, self-aware, deeply relatable.`;
    if (/dark humor|dark comedy|morbid|deadpan|absurd/i.test(input))
        return `
GENRE: Dark humor.
...
Short sentences. Flat delivery. Never explain the joke.`;
    if (/friendship|best friend|bestie|friends drifting|childhood friend|old friend/i.test(input))
        return `
GENRE: Friendship.
...
Tone: warm, specific, a little sad.`;
    if (/\bfamily\b|\bmom\b|\bdad\b|mother|father|parents|sibling|brother|sister|grandma|grandpa|childhood home/i.test(input))
        return `
GENRE: Family.
...
Tone: layered — love and frustration and grief can all be in one paragraph.`;
    if (/corporate|office|\bjob\b|\bwork\b|career|boss|layoff|startup|tech job|resignation|workplace|colleague|promotion/i.test(input))
        return `
...
Tone: a little bitter, tired, but still human underneath.`;
    if (/anxiety|stress|overthinking|panic attack|burnout|overwhelmed|spiral|can't stop thinking/i.test(input))
        return `
...
Tone: raw, claustrophobic, honest.`;
    if (/nostalgia|childhood memories|back then|used to|remember when|growing up|hometown|old days/i.test(input))
        return `
...
Tone: warm and a little sad. A little lost.`;
    if (/lonely|loneliness|\balone\b|invisible|disconnected|no one gets me|feel empty/i.test(input))
        return `
GENRE: Loneliness.
The worst loneliness is being around people and still feeling invisible.
Tone: quiet, interior, a little numb.`;
    if (/betrayal|backstab|trust broken|they lied|fake friend|used me|found out/i.test(input))
        return `
GENRE: Betrayal.
...
Tone: cold, sharp, controlled.`;
    if (/grief|loss|\bdeath\b|mourning|miss someone|they're gone|funeral|after they died/i.test(input))
        return `
GENRE: Grief / loss.
Grief hits people in normal moments — a grocery store, a song, seeing their handwriting.
Tone: tender, heavy, honest.`;
    if (/secret|confession|never told anyone|carrying this|nobody knows/i.test(input))
        return `
GENRE: Secret / confession.
Build the weight of the secret first — how long it has been carried.
Tone: close, a little dangerous.`;
    if (/motivat|comeback|never gave up|rock bottom|\brise\b|underdog|starting over after/i.test(input))
        return `
...
Tone: honest, gritty, earned. Never use hustle culture language.`;
    if (/adventure|road trip|travel|ran away|spontaneous|left everything/i.test(input))
        return `
...
The adventure changes something inside, not just outside.`;
    if (/obsess|can't stop thinking about|toxic love|fixated|checking their profile/i.test(input))
        return `
...
Tone: urgent, a little unhinged, dangerously honest.`;
    if (/healing|finding myself|new chapter|after everything|learning to/i.test(input))
        return `
...
Tone: quiet, a little fragile, determined.`;
    if (/parenting|new parent|\bfather\b|\bmother\b|\bbaby\b|\bkid\b|raising|my child|\bson\b|\bdaughter\b/i.test(input))
        return `
GENRE: Parenting.
...
Tone: tender, tired, full of love and doubt at the same time.`;
    if (/social media|followers|viral|influencer|cancel|instagram fame|online life/i.test(input))
        return `
...
Tone: a little dry, a little hollow, uncomfortably real.`;
    if (/\bschool\b|college|university|campus|\bexam\b|student|classroom|graduation/i.test(input))
        return `
...
function getFormatInstruction(input) {
...
End with a question or doubt — not a resolution.`;
    if (/diary|journal|dear diary/i.test(input))
        return `
...
End mid-thought or with a question the narrator can't answer.`;
    if (/voice note|voice memo|voice message/i.test(input))
        return `
FORMAT: Voice note.
...
End trailing off — not a conclusion.`;
    if (/text message|texting|text convo|chat story/i.test(input))
        return `
...
End on an image or action. Never an explanation.`;
    if (/monologue|stream of consciousness|inner monologue/i.test(input))
        return `
...
End in the middle of a thought — no conclusion.`;
    if (/unsent letter|letter to|open letter|dear [a-z]/i.test(input))
        return `
...
End without signing — or with something small and heartbreaking.`;
    if (/\bshort\b|brief|quick|7 lines|8 lines|9 lines/i.test(input))
        return `
...
One sensory detail. One emotional gut-punch. Done.`;
    if (/\blong\b|detailed|full story|extended|in depth/i.test(input))
        return `
...
function buildStoryPrompt(userInput, isFirstMessage) {
...
export async function* executeSunkarPipelineStream(prompt, sessionId = "default") {
    try {
        const history = getHistory(sessionId);
