...
export default function Create() {
  ...
  useEffect(() => {
    fetch(`${BACKEND_URL}/api/stories/history?userId=${USER_ID}`)
      .then(res => res.json())
      .then(data => setHistory(data.map((s: any) => s.generatedTitle || "Untitled").filter(Boolean)))
      .catch(() => setHistory([]));
  }, []);
  ...
  const handleGenerate = async () => {
    ...
    // Save current messages as context history BEFORE updating state
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/stories/generate-stream`, {
        ...
        body: JSON.stringify({ 
          prompt: userMessage, 
          userId: USER_ID,
          history: currentChatHistory // NOW SENDING PREVIOUS MESSAGES
        }),
      });
    ...
    <div className="flex h-screen w-full max-w-[1440px] mx-auto bg-[#000502] text-white p-4 md:p-6 gap-6 overflow-hidden">
      ...
      <aside className="w-80 hidden md:flex flex-col bg-emerald-950/10 border border-emerald-900/20 rounded-[2rem] p-6">
        ...
        <div className="flex-1 mt-8 overflow-y-auto custom-scrollbar">
          <p className="text-[10px] font-black uppercase tracking-widest text-emerald-800 mb-4 px-2">History</p>
        ...
        <div className="pt-6 border-t border-emerald-900/20 flex items-center gap-4">
          ...
          <div><p className="text-sm font-bold text-emerald-500">Sneha Jha</p></div>
        </div>
      </aside>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              <h1 className="text-5xl font-medium mb-4">Sunkar <span className="text-emerald-500 italic">GPT</span></h1>
              <p className="text-white/30 text-lg">What story do you want to tell today?</p>
            </div>
          ) : (
            messages.map((msg, idx) => (
              <div key={idx} className={`flex gap-4 animate-in fade-in slide-in-from-bottom-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex max-w-[85%] gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-emerald-600' : 'bg-emerald-500/20 text-emerald-500'}`}>
                    {msg.role === 'user' ? <User size={16} /> : <Bot size={16} />}
                  </div>
                  <div className={`p-4 rounded-2xl leading-relaxed whitespace-pre-wrap text-[15px] ${msg.role === 'user' ? 'bg-emerald-900/40 border border-emerald-500/30' : 'bg-white/5 border border-white/10'}`}>
                    {msg.content}
                    {msg.role === 'assistant' && !msg.content && loading && <Loader2 className="animate-spin text-emerald-500" size={18} />}
                  </div>
                </div>
              </div>
            ))
